import Card from "./components/card";
import CardHeader from "./components/card-header";
import CardBody from "./components/card-body";

import { Col, Row, Container } from "react-bootstrap";
import 'bootstrap/dist/css/bootstrap.min.css';
import Input from "./components/Input";
import Image from "./components/Image";
import Button from "./components/Button";
import {useState} from "react";
import Book from "./model/book";
import BookstoreService from "./service/BookstoreService";
export default function BookStore(){
    const IMG_QUESTION_MARK = '';
    const IMG_HEADER = 'data:image/png;base64,';
    let [book,setBook]=useState(new Book());
    let [books,setBooks]=useState(new Array())
    const bookStoreService=new BookstoreService();

    let toImage = (data) => {
       // console.log(data);
        if (data === null || data === undefined){
            return IMG_HEADER + IMG_QUESTION_MARK;
        }

        let array = data.split(',');
        if (array.length === 2) {
            return data;
        }
        return IMG_HEADER + data;
    };
    let toImageData = (img) => {
       // console.log(img);
        let array = img.split(',');
        if (array.length === 2) {
            return array[1];
        }
        return img;
    };
    function handleInputChange(event){
        const {name,value}=event.target;
        let newBook={...book};
        newBook[name]=value;
        setBook(newBook);
    }
    function handlePhotoChange(cover){
       // let newcover=toImageData(cover);
        setBook({...book,cover});
        //console.log(cover);

    }
    function findBookByIsbn(event){
        console.log(book.isbn);
        bookStoreService.findBookByIsbn(book.isbn)
            .then(setBook);

    }
    async function findBooks(event){
       // setBooks(await bookStoreService.findAllBooks());
        bookStoreService.findAllBooks().then(setBooks);
    }
    function addBook(event){
        //let newBook={...book};
        //newBook.cover=toImageData(book.cover);

        console.log("eklenecek kitap"+book.title);
        book.cover=toImageData(book.cover);
        console.log("eklenecek kitap"+book.cover);
        bookStoreService.addBook(book)
            .then(response=>{
                setBook(response);

            });
        console.log("eklenecek kitap"+book);
    }
    function deleteBook(event){
        bookStoreService.deleteBook(book.isbn)
            .then(setBook)
    }
    function updateBook(event){
        bookStoreService.updateBook(book.isbn,{...book})
            .then(setBook)

    }

    return(
        <Container>
            <Row>
                <div className="col-md-4 ">
                    <Row>
                        <label className="alert-success">Admin Panel</label>
                    </Row>
                    <Image id="cover"
                           handleFileChange={handlePhotoChange}
                           value={toImageData(book.cover)}
                           name="cover"
                           label="Cover">deneme</Image>
                    <Input id="isbn"
                           handleChange={handleInputChange}
                           value={book.isbn}
                           label="Isbn"></Input>
                    <Input id="author"
                           handleChange={handleInputChange}
                           value={book.author}
                           label="Author"></Input>
                    <Input id="title"
                           handleChange={handleInputChange}
                           value={book.title}
                           label="Title"></Input>
                    <Input id="pages"
                           handleChange={handleInputChange}
                           value={book.pages}
                           label="Pages"></Input>
                    <Input id="year"
                           handleChange={handleInputChange}
                           value={book.year}
                           label="Year"></Input>
                    <Input id="price"
                           handleChange={handleInputChange}
                           value={book.price}
                           label="Price"></Input>

                    <div className="input-group">
                        <span className="form-label"></span>
                        <Button id="find"
                                label="Find Book"
                                className="btn-success"
                                onClick={findBookByIsbn}
                               ></Button>
                        <span className="form-label"></span>
                        <Button id="findBooks"
                                label="Find Books"
                                className="btn-info"
                                onClick={findBooks}
                                ></Button>
                        <Button id="addBook"
                                label="Add Book"
                                className="btn-warning"
                                onClick={addBook}
                                ></Button>
                        <Button id="deleteBook"
                                label="Delete Book"
                                className="btn-danger"
                                onClick={deleteBook}
                               ></Button>
                        <Button id="updateBook"
                                label="Update Book"
                                className="btn-primary"
                                onClick={updateBook}
                                ></Button>
                    </div>
                </div>
                <div className="col-md-8  ">
                    <Row>
                        <label className="alert-success border-success text-center">Business Activity Monitor</label>
                    </Row>

                    <Card>
                        <CardHeader title="Books"></CardHeader>
                        <CardBody>
                            <table className="table table-hover table-striped table-bordered table-responsive">
                                <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Photo</th>
                                    <th>Isbn</th>
                                    <th>Title</th>
                                    <th>Author</th>
                                    <th>Price</th>
                                    <th>Pages</th>
                                    <th>Year</th>


                                </tr>
                                </thead>
                                <tbody>
                                {
                                    books.map( (b,idx) => (
                                            <tr key={b.isbn}>
                                                <td>{idx+1}</td>
                                                <td><img style={{width: '32px'}} src={toImage(b.cover)} /></td>
                                                <td>{b.isbn}</td>
                                                <td>{b.title}</td>
                                                <td>{b.author}</td>
                                                <td>{b.price}</td>
                                                <td>{b.pages}</td>
                                                <td>{b.year}</td>
                                            </tr>
                                        )
                                    )
                                }
                                </tbody>
                            </table>
                        </CardBody>
                    </Card>
                </div>

            </Row>





        </Container>

    );
}
